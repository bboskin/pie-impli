...
;; new forms
(define (make-λi loc body)
  (@ (syntax->srcloc loc) `(λi ,body)))

(define (make-Πi loc args body)
  (@ (syntax->srcloc loc) `(Πi ,args ,body)))
...
(define (parse-pie stx)
  (syntax-parse stx
    #:datum-literals (U
                      ...
                      → -> Pi Π the lambda λ Atom quote Πi λi ;; added
    ...
    [(Π ~! (~describe "argument names and types"
     ...
     (make-Pi stx
              ...
              (parse-pie #'B))]
    ;; new forms
    [(λi ~! b) (make-λi stx (parse-pie #'b))]
    [(Πi ~! (~describe "argument names and types"
                      ((x0:pie-id A0) (x:pie-id A) ...))
        (~describe "result type" B))
     (make-Πi stx
              (typed-binders (list (binding-site #'x0) (parse-pie #'A0))
...
(define-syntax (pie->binders expr)
  ...
  (syntax-parse stx
    #:datum-literals (U
                      Nat zero add1
                      which-Nat iter-Nat rec-Nat ind-Nat
                      → -> Pi Π the lambda λ  Atom quote λi Πi ;; added
                      cons car cdr Sigma Σ Pair
                      Trivial sole
                      List :: nil ind-List rec-List
                      Absurd ind-Absurd
    ...
    ;; new forms
    [(λi ~! b)
     (with-syntax ([lam/loc (syntax/loc (car (syntax-e stx)) kw:λi)]
                   [b/bindings #'(pie->binders b)])
       (add-disappeared (syntax/loc stx
                          (void lam/loc
                                b/bindings))
                        (car (syntax-e stx))))]
    [(Πi ~! ((x0:id A0) (x:id A) ...) B)
     (with-syntax ([pi/loc (syntax/loc (car (syntax-e stx)) kw:Πi)]
                   [A0* #'(pie->binders A0)]
                   [(A* ...) #'((pie->binders A) ...)]
                   [B* #'(pie->binders B)])
       (add-disappeared (syntax/loc stx
