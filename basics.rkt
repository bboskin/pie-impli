...
(provide (all-defined-out)
         ...
         location->srcloc
         Srcloc
         ;; added provide to be used by inference
         (struct-out @)
         (struct-out binder)
         #;structs->exp
         #;inf->tv)
...
(define-type Src-Stx
  (U (List 'the Src Src)
     ...
     (List 'Πi (List* Typed-Binder (Listof Typed-Binder)) Src)
...
(define-type Core
  (U (List 'the Core Core)
     ...
     ;; new form
     (List 'Πi (List (List Symbol Core)) Core)
...
(define Core? (make-predicate Core))
...
(struct PI ([arg-name : Symbol]
  ...
  #:transparent)
;; new form
(struct PIi ([arg-name : Symbol]
             [arg-type : Value]
             [result-type : Closure])
  #:transparent)
(struct LAM ([arg-name : Symbol] [body : Closure]) #:transparent)
(struct LAMi ([body : Value]) #:transparent)
...
(define-here-and-for-syntax (var-name? x)
  (not (or (eqv? x 'U) (eqv? x 'Nat) (eqv? x 'zero)
           ...
           (eqv? x '->) (eqv? x '→) (eqv? x 'Π) (eqv? x 'Πi) (eqv? x 'Pi) (eqv? x 'λ) (eqv? x 'λi) (eqv? x 'lambda)
